  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      ...
      re.explainingEquation = (r) => {
        return `min(${A.getItem([i, j]).makeElem().outerHTML}, ${B.getItem([i, j]).makeElem().outerHTML} = ${r.makeElem().outerHTML}`;
      };
    }
  }
  return result;
...
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.minimum.html">torch.minimum</a>(A, B)</code><br/>
`;
explanation = `
  ...
  minimum of the items at the same indices in <code>A</code> and <code>B</code>, or <code>nan</code> if either item is <code>nan</code>.</p>
  </div>
`;
var fxnArgs = {};
let A = new Tensor("A", [
