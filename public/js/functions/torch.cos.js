...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      ...
      re.value = Math.cos(input.getItem([i, j]).value).toFixed(3);
      re.addRelation(input, [i, j]);
      re.explainingEquation = (r) => `cos(${input.getItem([i, j]).makeElem().outerHTML} radians) = ${r.makeElem().outerHTML}`;
    }
  }
  return result;
}
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.cos.html">torch.cos</a>(input)</code><br/>
`;
explanation = `
  <div class="explanation">
  <p><code>cos</code> calculates the element-wise tangent of the input tensor.</p>
  </div>
`;
var fxnArgs = {};
