...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      ...
      re.value = Math.tanh(A.getItem([i, j]).value).toFixed(3);
      re.addRelation(A, [i, j]);
      re.explainingEquation = (r) => `tanh(${A.getItem([i, j]).makeElem().outerHTML}) = ${r.makeElem().outerHTML}`;
    }
  }
  return result;
}
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.tanh.html">torch.tanh</a>(A)</code><br/>
`;
explanation = `
  <div class="explanation">
  <p><code>tanh</code> performs the element-wise hyperbolic tangent of the input tensor.</p>
  </div>
`;
var fxnArgs = {};
let A = new Tensor("A", [
