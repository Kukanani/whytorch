...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      ...
      if (B.getItem([i, j]).value === 0) {
        re.value = NaN;
      }
      else {
        re.value = (A.getItem([i, j]).value / B.getItem([i, j]).value).toFixed(2);
      }
...
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.divide.html">torch.divide</a>(A, B)</code><br/>
  <code>ans = A / B</code> <span class="codeAnnotation">(equivalent)</span><br/>
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.div.html">torch.div</a>(A, B)</code><span class="codeAnnotation">(equivalent)</span><br/>
`;
explanation = `
  ...
  item at the <code>i</code>th row and <code>j</code>th column of <code>A</code> divided by
