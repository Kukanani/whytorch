...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      ...
      if (other.getItem([i, j]).value === 0) {
        re.value = NaN;
      }
      else {
        re.value = (input.getItem([i, j]).value / other.getItem([i, j]).value).toFixed(2);
      }
...
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.divide.html">torch.divide</a>(input, other)</code><br/>
  <code>ans = input / other</code> <span class="codeAnnotation">(equivalent)</span><br/>
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.div.html">torch.div</a>(input, other)</code><span class="codeAnnotation">(equivalent)</span><br/>
`;
explanation = `
  <div class="explanation">
  <p><code>divide</code> performs element-wise division of two tensors.</p>
  <p>The item in the <code>i</code>th row and <code>j</code>th column of <code>ans</code> is the
  item at the <code>i</code>th row and <code>j</code>th column of <code>input</code> divided by
