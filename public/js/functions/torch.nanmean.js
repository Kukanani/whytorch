...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j < m; j++) {
      ...
      if(isNaN(input.getItem(coords).value)) {
        continue;
      }
...
explanation = `
  ...
  <p><code>nanmean</code> finds the mean (average) of a tensor along a dimension, discarding <code>nan</code> values.</p>
  <p>The <code>i</code>th item in <code>ans</code> is the
  mean (average) of all items of <code>input</code> along the <code>dim</code>th dimension.</p>
  </div>
`;
var fxnArgs = {};
