...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      ...
      re.value = multiplyArray(itemsToGroup.map(x => x.value));
...
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.cumprod.html">torch.cumprod</a>(input, dim)</code><br/>
`;
explanation = `
  <div class="explanation">
  <p><code>cumprod</code> accumulates the product of values along a dimension.</p>
  <p>The item in the <code>i</code>th position of <code>dim</code> in <code>ans</code> is the product of all the items
