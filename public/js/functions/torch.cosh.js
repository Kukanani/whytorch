...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < m; j++) {
      ...
      re.value = Math.cosh(input.getItem([i, j]).value).toFixed(3);
      re.addRelation(input, [i, j]);
      re.explainingEquation = (r) => `cosh(${input.getItem([i, j]).makeElem().outerHTML}) = ${r.makeElem().outerHTML}`;
    }
  }
  return result;
}
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.cosh.html">torch.cosh</a>(input)</code><br/>
`;
explanation = `
  <div class="explanation">
  <p><code>cosh</code> calculates the element-wise hyperbolic cosine of the input tensor.</p>
  </div>
`;
var fxnArgs = {};
