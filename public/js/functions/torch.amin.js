...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    result.getItem([i]).explainingEquation = (r) => {
      return `min(${addTermsHtml}) = ${r.makeElem().outerHTML}`;
    }
  }
  return result;
}
sourceCode = `
  <code>ans = <a href="https://docs.pytorch.org/docs/stable/generated/torch.amin.html">torch.amin</a>(A, dim)</code><br/>
`;
explanation = `
  <div class="explanation">
  <p><code>amin</code> finds the min of a tensor along a dimension.</p>
  <p>The <code>i</code>th item in <code>ans</code> is the
  minimum of all items of <code>A</code> along the <code>dim</code>th dimension.</p>
  </div>
`;
var fxnArgs = {};
let A = new Tensor("A", [
