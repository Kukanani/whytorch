  ...
  if (dimVal === 0) {
    for (let i = 0; i < index.items.length; i++) {
      ...
      for (let j = 0; j < index.items[i].length; j++) {
        ...
        re.explainingEquation = (r) => {
          return `input[${index.getItem([i, j]).makeElem(d=dim).outerHTML}, ${j}] = ` + input.getItem([index.getItem([i, j]).value, j]).makeElem().outerHTML + " → " + r.makeElem().outerHTML;
        };
      }
    }
  } else if (dimVal === 1) {
    for (let i = 0; i < index.items.length; i++) {
      ...
      for (let j = 0; j < index.items[i].length; j++) {
        ...
        re.addRelation(dim, []);
        re.explainingEquation = (r) => {
          return `input[${i}, ${index.getItem([i, j]).makeElem(d=dim).outerHTML}] = ` + input.getItem([i, index.getItem([i, j]).value]).makeElem().outerHTML + " → " + r.makeElem().outerHTML;
        };
      }
    }
  }
