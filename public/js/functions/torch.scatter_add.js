...
function do_function(fxnArgs) {
  ...
  for (let i = 0; i < index.items.length; i++) {
    for (let j = 0; j < index.items[i].length; j++) {
      ...
      var inputitem = input.getItem(coords);
      ritem.value = sitem.value + inputitem.value;
      ...
      ritem.explainingEquation = (r) => {
        ...
        coords[dimVal] = iitem.value;
        var inputitem = input.getItem(coords);
        return `src[${iitem.makeElem(d=dim).outerHTML}, ${j}] = ${sitem.makeElem().outerHTML} + ${inputitem.makeElem().outerHTML} = ${r.makeElem().outerHTML}`;
      };
    }
  }
...
const sourceCode = `
  <code>ans = <a href=\"https://pytorch.org/docs/stable/generated/torch.scatter_add.html\">torch.scatter_add</a>(input, dim, index, src)</code><br/>
`;
const explanation = `
  <p><code>scatter</code> adds values from <code>src</code> into <code>input</code> to the values from <code>input</code>
  at the indices specified by <code>index</code> along dimension <code>dim</code>.</p>
  <p>For each location in <code>index</code>, the result contains the original value of <code>input</code>
  plus the value from <code>src</code> at that index.</p>
`;
